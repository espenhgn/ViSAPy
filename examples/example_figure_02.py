...
if RANK == 0:
    #savefolder = glob.glob('savedata_figure_02*')[-1]
    string = asctime().split()
    savefolder = os.path.join(os.path.split(__file__)[0], 'savedata_figure_02_')
    for s in string:
        for ss in s.split(':'):
            savefolder += ss + '_'
...
cellParameters = {
    ...
    'timeres_python' : 2**-5,
    'tstartms' : 0.,
    'tstopms' : 1500, #1 second without 500 ms startup transient
    'verbose' : False,
    'pt3d' : False,
}
...
myplot.colors = ['0.3', 'r', 'b']
fig = myplot.plot_figure_02(cells=benchmark_data.read_lfp_cell_files())
fig.savefig(os.path.join(savefolder, 'figure_02.pdf'), dpi=150)
